...
setwd("~/")
  ...
  ind.tus15<-read_dta("Data/tus/UKDA-8128-stata/stata/stata11_se/uktus15_individual.dta")%>%
  ...
  ind.tus15<-droplevels(merge(ind.tus15,read_dta("Data/tus/agekidvars_16.dta"),by="serial", all.x=T,all.y=F))				
  ...
  ind.tus15<-ind.tus15<-merge(ind.tus15,
		  read_dta("Data/tus/UKDA-8128-stata/stata/stata11_se/uktus15_household.dta")%>%
...
ind.tus15$pid<-((ind.tus15$serial*100)+ind.tus15$pnum)
  ...
  ind.tus15<-merge(ind.tus15%>%select(-dhhtype),
		     read_dta("Data/tus/hhtype.dta")%>%select(dhhtype3,serial),by="serial", all.x=T,all.y=F)				
...
ep.chap15<-read_dta("Data/tus/epis_chap0015.dta") %>% filter(year==2015 ) %>% select(-'what_oth1')
  ...
  ep.chap15<-merge(ep.chap15,
  		            read_dta("Data/tus/UKDA-8128-stata/stata/stata11_se/uktus15_diary_ep_long.dta")%>% 
...
hh.t$car.aln.rat<-ifelse(hh.t$car.aln.rat>1,1,hh.t$car.aln.rat)
  ...
  hh.t<- hh.t %>% mutate(
  		car.pri.cat = case_when(car.pri.rat == 0 ~ " Not caring",car.pri.rat> 0 & car.pri.rat< .4 ~ '<40%',
  		                        car.pri.rat>=.4 & car.pri.rat<1~'>=40%', car.pri.rat>=1  ~ "M don't care"),
  		car.sec.cat = case_when(car.sec.rat == 0 ~ " Not caring",car.sec.rat> 0 & car.sec.rat< .4 ~ '<40%',
  		                        car.sec.rat>=.4 & car.sec.rat<1~'>=40%', car.sec.rat>=1   ~ "M don't care"),
  		car.ann.cat = case_when(car.ann.rat == 0 ~ " Not caring",car.ann.rat> 0 & car.ann.rat< .4 ~ '<40%',
  		                        car.ann.rat>=.4 & car.ann.rat<1~'>=40%', car.ann.rat>=1   ~ "M don't care"),
  		car.anw.cat = case_when(car.anw.rat == 0 ~ " Not caring",car.anw.rat> 0 & car.anw.rat< .4 ~ '<40%',
  		                        car.anw.rat>=.4 & car.anw.rat<1~'>=40%', car.anw.rat>=1   ~ "M don't care"),
  		car.aln.cat = case_when(car.aln.rat == 0 ~ " Not caring",car.aln.rat> 0 & car.aln.rat< .4 ~ '<40%',
  		...
  		car.pri.cat3 = case_when(car.pri.rat == 0 ~ " Not caring",car.pri.rat> 0 & car.pri.rat< .4 ~ '<40%',
  		                         car.pri.rat>=.4~'>=40%'),
  		car.sec.cat3 = case_when(car.sec.rat == 0 ~ " Not caring",car.sec.rat> 0 & car.sec.rat< .4 ~ '<40%',
  		                         car.sec.rat>=.4~'>=40%'),
  		car.ann.cat3 = case_when(car.ann.rat == 0 ~ " Not caring",car.ann.rat> 0 & car.ann.rat< .4 ~ '<40%',
  		                         car.ann.rat>=.4~'>=40%'),
  		car.anw.cat3 = case_when(car.anw.rat == 0 ~ " Not caring",car.anw.rat> 0 & car.anw.rat< .4 ~ '<40%',
  		                         car.anw.rat>=.4~'>=40%'),
  		car.aln.cat3 = case_when(car.aln.rat == 0 ~ " Not caring",car.aln.rat> 0 & car.aln.rat< .4 ~ '<40%',
  		                         car.aln.rat>=.4~'>=40%'),
  		car.spo.cat3 = case_when(car.spo.rat == 0 ~ " Not caring",car.spo.rat> 0 & car.spo.rat< .4 ~ '<40%',
  		                         car.spo.rat>=.4~'>=40%'),
  		car.rou.cat3 = case_when(car.rou.rat == 0 ~ " Not caring",car.rou.rat> 0 & car.rou.rat< .4 ~ '<40%',
  		                         car.rou.rat>=.4~'>=40%'),
  		car.enr.cat3 = case_when(car.enr.rat == 0 ~ " Not caring",car.enr.rat> 0 & car.enr.rat< .4 ~ '<40%',
		...
		car.pri.q3.d=ntile(car.pri.t.d,3),	car.pri.q3.m=ntile(car.pri.t.m,3), car.pri.q4.d=ntile(car.pri.t.d,4),	
		                            car.pri.q4.m=ntile(car.pri.t.m,4),
		car.rou.q3.d=ntile(car.rou.t.d,3),	car.rou.q3.m=ntile(car.rou.t.m,3), car.rou.q4.d=ntile(car.rou.t.d,4),	
		car.rou.q4.m=ntile(car.rou.t.m,4),
		car.enr.q3.d=ntile(car.enr.t.d,3),	car.enr.q3.m=ntile(car.enr.t.m,3), car.enr.q4.d=ntile(car.enr.t.d,4),	
		car.enr.q4.m=ntile(car.enr.t.m,4),
		car.aln.q3.d=ntile(car.aln.t.d,3),	car.aln.q3.m=ntile(car.aln.t.m,3), car.aln.q4.d=ntile(car.aln.t.d,4),	
		car.aln.q4.m=ntile(car.aln.t.m,4),		
)
